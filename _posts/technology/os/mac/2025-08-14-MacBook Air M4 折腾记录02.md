...
最近玩 claude code 通过 hooks 触发通知，这样就可以知道 claude code 需要授权的时候去确认一下。

但是脚本执行后没有通知。

参考：https://discussionschinese.apple.com/thread/256121612?answerId=261532569322&sortBy=rank#261532569322

所以得先执行一个通知脚本，触发通知权限。

直接脚本编辑器新建脚本后运行下面的脚步通知就可以了

```ABAP
display notification "这是一个简单的Mac脚本通知！" with title "脚本提醒"
```
